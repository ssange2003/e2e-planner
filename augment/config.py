...
# [미검증 — 데이터로 확정 불가] ESC breakaway.
#
# 수집 데이터만으로는 검증도 반박도 되지 않아 값을 그대로 둔다.
# 0.50 이상 정밀 히스토그램(2240행, bin 0.025):
#   0.750~0.775   49
#   0.775~0.800  128
#   0.800~0.825  131
#   0.825~0.850  354   <- 최빈
#   0.850~0.875  250
#   0.875~0.900  145
# 임계값 0.85 가 최빈 봉우리 바로 위에 박혀 있다. 자연적인 골(gap)이 없어
# 0.84 로 한 칸만 내려도 수백 프레임이 한꺼번에 판정을 뒤집는다.
# (대조: ZERO_EVENT_THRESH 는 0.001~0.10 이 완전 공백이라 어디 둬도 동일)
#
# 또 이 값이 참이면 주행 데이터의 29.6%(0.75~0.85)가 정지 상태여야 하는데
# 주행 로그로서 앞뒤가 맞지 않는다.
#
# [확정 방법] IMU 수집 후 아래를 실행하면 데이터가 직접 답을 준다.
#     python augment/imu_check.py data/<새파일>.csv
# 스로틀 구간별 "실제로 움직인 비율" 곡선의 무릎(knee)이 곧 breakaway 다.
...
# [실측] 절대 위협 거리.
#
# 구값 0.30 은 4개 파일 2240행 전체에서 단 한 번도 발화하지 않았다.
# 정면 s2 의 1퍼센타일이 s_curve 0.406m / avoidance 0.391m 이라
# 정면이 0.30m 보다 가까워지는 상황 자체가 존재하지 않았다.
# (운전자가 그 전에 회피하거나 멈춤 — 사람 반응이 임계값보다 빠르다)
# 즉 STOP 판정의 1차 조건이 죽은 코드였다.
#
# 후보값별 "정면 < 임계값" 발화율 실측:
#            normal_c1  normal_c2   s_curve  avoidance
#   0.30        0.0%       0.0%       0.0%      0.0%   <- 완전 무반응
#   0.40        0.0%       0.0%       0.8%      1.9%
#   0.45        0.0%       0.0%       2.7%      2.4%   <- 채택
#   0.50        0.1%       0.0%       6.5%      5.7%   <- 개활지 오탐 시작
#
# 0.45 는 개활 코스가 정확히 0.0% 를 유지하는 마지막 값이면서 장애물
# 코스는 반응하는 지점이다. 한 칸 더 올리면 normal_c1 이 새기 시작한다.
#
# [주의] 라이다 마운트가 센서 원점 기준인지 범퍼 오프셋 보정값인지는
# 여전히 미확인. 차선폭 역산 53cm vs 제시값 40cm 차이가 남아 있다.
LIDAR_DANGER_M = 0.45
...
# [실측] 정면이 이 이상이면 "진행 공간 있다".
#
# 구값 0.35 는 4개 파일에서 99.7~100% 가 통과했다. "정면이 열려 있는가" 라는
# 조건이 항상 참이라 S구간 판정에 아무 기여도 하지 못했고, 실질적으로
# "양쪽이 가까운가" 하나로만 판정되고 있었다.
#
# 후보값별 "정면 >= 임계값" 통과율 실측:
#            normal_c1  normal_c2   s_curve  avoidance
#   0.35      100.0%     100.0%      99.7%    100.0%   <- 항상 참, 무의미
#   0.45      100.0%     100.0%      97.3%     97.6%   <- 채택
#   0.60       98.7%     100.0%      84.9%     81.9%   <- 정상 통과분까지 배제
#
# LIDAR_DANGER_M 과 같은 0.45 로 맞춘 것은 의도적이다. 경계를 하나로 두면
#   정면 <  0.45  = 막혔다(위협)
#   정면 >= 0.45  = 열렸다(S구간 통과 가능)
# 로 두 판정이 모순 없이 맞물린다. 서로 다른 값이면 "위협이면서 동시에
# 열려 있는" 프레임이 생겨 STOP 과 S_CURVE 가 함께 켜진다.
S_CURVE_FRONT_OPEN_M = 0.45
S_CURVE_REQUIRED_RATIO = 0.60   # 윈도우 내 이 비율 이상 지속되어야 S구간 인정
S_CURVE_WINDOW = 5              # S구간 판정 스무딩 윈도우

# ─────────────────────────────────────────────────────────────────────────────
# IMU (D435i)  — 게임패드 글리치 판별의 직접 증거
# ─────────────────────────────────────────────────────────────────────────────
#
# [왜 필요한가]
# throttle=0 이 찍힌 프레임에는 두 종류가 섞여 있습니다.
#   (a) 게임패드 데드존 글리치 — 운전자는 계속 주행 중, 차는 관성으로 굴러감
#   (b) 운전자가 실제로 멈춤   — 진짜 STOP
# 이 둘은 차 바깥 장면이 완전히 동일합니다. 그래서 라이다도 카메라도
# 원리적으로 구분할 수 없고, 지금까지는 "정면에 장애물이 있었나" 라는
# 간접 추론에 의존해 왔습니다. 개활지에서 그냥 멈춘 경우는 놓칩니다.
# IMU 는 "차가 실제로 움직이는가" 를 직접 관측하므로 이 문제를 정면으로 풉니다.
#
# [왜 적분하지 않는가]
# 가속도를 적분해 속도를 구하면 바이어스 드리프트로 수 초 만에 무의미해집니다.
# 대신 |accel| 의 표준편차(노면 진동 세기)를 봅니다. 차가 구르면 값이 흔들리고
# 멈추면 거의 상수가 됩니다. 누적 오차가 원리적으로 존재하지 않습니다.

# [미검증] 이동/정지 판정 임계값. 반드시 실측으로 정하세요.
#     python camera.py --imu-check
# 를 정지 상태에서 실행하면 "정지 상태 motion" 값이 출력됩니다.
# 그 값보다 충분히 큰 값(예: 3~5배)을 여기에 넣으십시오.
# 노면 재질과 서스펜션에 따라 크게 달라지므로 기본값은 임시값입니다.
IMU_MOTION_THRESH = 0.15      # 이 값보다 크면 "움직이는 중"

# [미검증] 회전 판정 임계값 [rad/s].
# 게임패드 조향이 {0, ±0.9} 로 이산적이라 조향값으로는 코너링을 못 가립니다.
# gyro 는 실제 각속도를 주므로 이 문제를 우회합니다. 0.3 rad/s ~= 17°/s.
IMU_YAW_THRESH = 0.30

# IMU 컬럼이 존재해도 전부 0 이면 (IMU 미탑재 기기에서 0 으로 채워진 경우)
# 증거로 쓰지 않습니다. 파일 단위로 이 비율 이상이 0 이면 미탑재로 간주합니다.
IMU_ALLZERO_RATIO = 0.99

IMU_COLS = ["imu_motion", "imu_yaw_rate", "imu_accel_fwd"]
